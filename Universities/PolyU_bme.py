...
def getinfo():
    ...
    if os.path.isfile("./Data/PolyU_bme.xlsx"):
        return None

    driver = webdriver.Chrome()
    driver.get('https://www.polyu.edu.hk/bme/people/academic-staff/')
    ...
    for i in range(len(pros)):
        ...
        if "Research" in title[0].text:
            driver.back()
            pros = driver.find_elements(By.XPATH,
                                        '//span[@class="ppl-detail-blk__name theme-color-text underline-link__line"]')
            continue
        if info and name != "Lei Sun":
    ...
    df.loc[len(df)] = ["Thomas Lee", "", "", "", "The Hong Kong University of Science and Technology"]
    df.loc[len(df)] = ["Ir Prof. Ming Zhang", "", "", "", "King's College London"]
    df.loc[len(df)] = ["Ir Prof. Yongping Zheng", "", "", "", "PolyU"]
    df.loc[len(df)] = ["Prof. Mo Yang", "", "", "", "University of California, Riverside"]
    df.loc[len(df)] = ["Prof. Lei Sun", "", "", "", "Pennsylvania State University"]

    df.to_excel('./Data/PolyU_bme.xlsx', index=False)
