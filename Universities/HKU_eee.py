...
def getinfo():
    ...
    if os.path.isfile("./Data/HKU_eee.xlsx"):
        return None

    driver = webdriver.Chrome()
    driver.get('https://www.eee.hku.hk/people/')
    df = pd.DataFrame(columns=['name', 'describe', 'trans', 'split', 'PhD'])

    all = driver.find_elements(By.XPATH, '//h5[@class="heading"]')

    for i in range(len(all)):
        driver.execute_script('arguments[0].click();', all[i])
        time.sleep(3)
        name = driver.find_elements(By.XPATH, '//h3')[0].text
        info = driver.find_elements(By.XPATH, '//div[@class="entry"]/p')
        for t in info:
            text = t.text
    ...
    df.to_excel('./Data/HKU_eee.xlsx', index=False)
